...
This is the FINAL synthesized version incorporating all triplet feedback:
1. Threading fix for asyncio.run() (all triplets)
2. Turn counter fix for multiple responses (Gemini)
3. Flattened data structure matching schema (Gemini)
4. Full text storage for responses (DeepSeek)
"""
import os
import yaml
...
def log_conversation(
    ...
    """
    Log conversation turn to Godot (non-blocking, thread-safe).

    Args:
        conversation_id: UUID for this conversation
        turn_number: 1 for request, 2+ for responses
        role: 'user' or 'assistant'
        content: Full prompt or full response text
    ...
    Note:
        ...
        Uses thread-safe execution to avoid asyncio event loop conflicts.
    """
    try:
        # Prepare log data (FLATTENED structure matching DB schema)
        log_data = {
        ...
        # Populate metadata with optional fields
        metadata = {}
        if files:
        ...
        # Thread-safe async execution (avoids event loop conflicts)
        def _send_log():
            try:
        ...
        # Start daemon thread (fire and forget, non-blocking)
        thread = threading.Thread(target=_send_log, daemon=True)
        thread.start()

    except Exception:
        # Silently fail on any error in preparation
        pass
