...
async function spawnClaude(command, options = {}, ws) {
  return new Promise(async (resolve, reject) => {
    ...
    console.log('🔍 Using Claude CLI path:', claudeCliPath);

    // Parse CLAUDE_CLI_PATH - it might be a multi-part command like "docker exec -i container claude"
    let claudeCommand, claudeArgs;
    if (claudeCliPath.includes(' ')) {
      // Split the command into parts
      const parts = claudeCliPath.split(' ');
      claudeCommand = parts[0];
      claudeArgs = [...parts.slice(1), ...args];
      console.log('🔍 Parsed command:', claudeCommand, 'with args:', claudeArgs);
    } else {
      claudeCommand = claudeCliPath;
      claudeArgs = args;
    }

    const claudeProcess = spawnFunction(claudeCommand, claudeArgs, {
      cwd: workingDir,
